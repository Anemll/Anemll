...
gloabl_compute_precision=ct.precision.FLOAT16
#gloabl_compute_precision=ct.precision.FLOAT32
...
class Qwen25Converter(BaseConverter):
    ...
    def postprocess(self, num_workers=None):
        ...
        if self.converted_model is not None and self.lut_bits is not None:
            print(
                f"Applying LUT quantization with {self.lut_bits} bits per tensor "
            )
            try:
    ...
    def convert_to_coreml(self, model: Qwen25ForCausalLM) -> ct.models.MLModel:
        ...
        mlmodel = ct.convert(
            ...
            compute_precision=gloabl_compute_precision,
