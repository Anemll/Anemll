...
# Advanced chat interface with sampling parameters
...
def sample_top_k(probs, k):
    ...
    top_k_probs, top_k_indices = torch.topk(probs, k=min(k, probs.size(-1)))
...
def sample_top_p(probs, p):
    ...
    # Find cutoff
    cutoff_index = torch.searchsorted(cumulative_probs, p, right=True)
    if cutoff_index < len(probs):
        cutoff_index += 1
    
    # Zero out tokens below cutoff
    probs_filtered = torch.zeros_like(probs)
    probs_filtered[sorted_indices[:cutoff_index]] = sorted_probs[:cutoff_index]
...
def generate_next_token(embed_model, ffn_models, lmhead_model, input_ids, pos, context_length, 
                       metadata, state=None, causal_mask=None, temperature=0.7, top_k=50, 
                       top_p=0.95, repetition_penalty=1.1, generated_tokens=None):
    ...
    if repetition_penalty != 1.0 and generated_tokens is not None and len(generated_tokens) > 0:
        logits = apply_repetition_penalty(logits, torch.tensor(generated_tokens), repetition_penalty)
...
def chat_loop(embed_model, ffn_models, lmhead_model, tokenizer, metadata, state, causal_mask=None, 
              auto_prompt=None, warmup=False, save_file=None, max_tokens=None, no_template=False, 
              eval_mode=False, temperature=0.7, top_k=50, top_p=0.95, repetition_penalty=1.1,
              do_sample=True):
    """Interactive chat loop with advanced sampling."""
    ...
    try:
        while True:
            ...
            try:
                ...
                # Set temperature based on do_sample flag
                effective_temp = temperature if do_sample else 0.0
                
                while pos < context_length - 1:
                    # Generate next token with advanced sampling
...
def parse_args():
    parser = argparse.ArgumentParser(description='Advanced chat with CoreML LLaMA with sampling parameters (c) 2025 Anemll')
    ...
    parser.add_argument('--top-k', type=int, default=50,
                       help='Top-k filtering for sampling (default: 50, 0 = disabled)')
    parser.add_argument('--top-p', type=float, default=0.95,
                       help='Top-p (nucleus) filtering for sampling (default: 0.95)')
